...
# Handles standard, DLP, and Nanopore projects in one pass.
...
def link_by_request(reqID):
    ...
    if isDLP:
        # DLP links one symlink per fastq directory (run-level), not per sample.
        ...
        for fastq_dir in fastq_directories:
            ...
            slink = fastq_dir
            # -n: replace existing symlink-to-directory in place instead of
            # dereferencing and writing inside the source.
            cmd = "ln -sfn {} {}".format(slink, dlink)
            logger.info(cmd)
            subprocess.run(cmd, shell=True)
    elif request_name == "Nanopore":
        # Nanopore data lives under /igo/delivery/nanopore/Project_<id>; link the whole project folder.
